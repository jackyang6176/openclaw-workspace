...
def generate_html_report(weather_data, date_str):
    """生成 HTML 報告 (修復版本 - 無跳轉)"""
    # 轉義 HTML 特殊字符
    escaped_weather = weather_data.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
...
<body>
    <div class="container">
        ...
        <h2>天氣預報</h2>
        <div class="weather-data">{escaped_weather}</div>
...
def copy_to_nginx(html_content, date_str):
    """複製文件到 Nginx 目錄"""
    nginx_dir = "/usr/share/nginx/html/travel/wuling-farm/weather"
    os.makedirs(nginx_dir, exist_ok=True)
    
    # 寫入日期報告
    date_file = f"{nginx_dir}/{date_str}.html"
    with open(date_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    # 寫入 latest.html
    latest_file = f"{nginx_dir}/latest.html"
    with open(latest_file, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print(f"已部署到 Nginx: {date_file}")
...
def main():
    ...
    # 部署到 Nginx
    copy_to_nginx(html_report, date_str)
    
    print(f"武陵農場氣象報告已生成:")
    print(f"Discord 報告: {markdown_path}")
    print(f"網站報告已部署到 Nginx")

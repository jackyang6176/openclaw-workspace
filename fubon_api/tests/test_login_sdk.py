#!/usr/bin/env python3
"""
富邦 Neo API - sdk.login 帳號密碼登入測試
使用 sdk.login(user_id, password) 方法
"""
import os
import sys
import json
...
def log_message(message, level="INFO"):
    ...
    # 寫入日誌檔案
    log_file = os.path.join(LOG_DIR, f"sdk_login_test_{datetime.now().strftime('%Y%m%d')}.log")
...
def test_sdk_login():
    """測試 sdk.login 帳號密碼登入"""
    print("=" * 70)
    print("富邦 Neo API - sdk.login 帳號密碼登入測試")
    print("=" * 70)
    ...
    # 使用 sdk.login 帳號密碼登入
    log_message("=" * 70)
    log_message("執行 sdk.login(user_id, password)...")
    log_message("=" * 70)
    
    try:
        # 使用帳號密碼登入（不使用憑證）
        result = sdk.login(user_id, password)
        ...
        # 記錄詳細錯誤到日誌
        error_log = os.path.join(LOG_DIR, f"sdk_login_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def main():
    ...
    print("=" * 70)
    if success:
        print("✅ 測試完成：sdk.login 帳號密碼登入成功")
    else:
        print("❌ 測試完成：sdk.login 帳號密碼登入失敗")
    print("=" * 70)

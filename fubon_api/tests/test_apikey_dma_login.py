#!/usr/bin/env python3
"""
富邦 Neo API - apikey_dma_login 登入測試
使用 sdk.apikey_dma_login(user_id, api_key) 方法
這是 DMA 模式，不需要憑證！
"""
import os
import sys
...
def log_message(message, level="INFO"):
    ...
    # 寫入日誌檔案
    log_file = os.path.join(LOG_DIR, f"apikey_dma_login_test_{datetime.now().strftime('%Y%m%d')}.log")
    with open(log_file, 'a', encoding='utf-8') as f:
        f.write(log_line + '\n')

def test_apikey_dma_login():
    """測試 apikey_dma_login 登入（DMA 模式，無需憑證）"""
    print("=" * 70)
    print("富邦 Neo API - apikey_dma_login 登入測試")
    print("=" * 70)
    print("模式：DMA（Direct Market Access）- 無需憑證")
    ...
    try:
        ...
        if not user_id:
            log_message("配置文件中缺少 user_id", "ERROR")
            return False
    ...
    # 使用 apikey_dma_login 登入
    log_message("=" * 70)
    log_message("執行 sdk.apikey_dma_login(user_id, api_key)...")
    log_message("=" * 70)
    
    try:
        # 使用 API Key DMA 模式登入（無需憑證）
        result = sdk.apikey_dma_login(user_id, api_key)
        ...
        if hasattr(result, 'is_success') and result.is_success:
            ...
            log_message("❌ API Key DMA 登入失敗", "ERROR")
        ...
        # 記錄詳細錯誤到日誌
        error_log = os.path.join(LOG_DIR, f"apikey_dma_login_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def main():
    """主程式"""
    success = test_apikey_dma_login()
    
    print()
    print("=" * 70)
    if success:
        print("✅ 測試完成：apikey_dma_login 登入成功")
    else:
        print("❌ 測試完成：apikey_dma_login 登入失敗")
    print("=" * 70)

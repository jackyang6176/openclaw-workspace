#!/usr/bin/env python3
"""
富邦 Neo API - apikey_login 登入測試
使用 sdk.apikey_login(user_id, api_key) 方法
"""
import os
import sys
import json
...
def log_message(message, level="INFO"):
    ...
    # 寫入日誌檔案
    log_file = os.path.join(LOG_DIR, f"apikey_login_test_{datetime.now().strftime('%Y%m%d')}.log")
...
def test_apikey_login():
    """測試 apikey_login 登入"""
    print("=" * 70)
    print("富邦 Neo API - apikey_login 登入測試")
    print("=" * 70)
    ...
    # 使用 apikey_login 登入
    log_message("=" * 70)
    log_message("執行 sdk.apikey_login(user_id, api_key)...")
    log_message("=" * 70)
    
    try:
        # 使用 API Key 登入（v2.2.7+）
        result = sdk.apikey_login(user_id, api_key)
        ...
        # 記錄詳細錯誤到日誌
        error_log = os.path.join(LOG_DIR, f"apikey_login_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def main():
    """主程式"""
    success = test_apikey_login()
    
    print()
    print("=" * 70)
    if success:
        print("✅ 測試完成：apikey_login 登入成功")
    else:
        print("❌ 測試完成：apikey_login 登入失敗")
    print("=" * 70)

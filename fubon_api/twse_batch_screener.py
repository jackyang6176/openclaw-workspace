...
def batch_screen(symbols: List[str], batch_size: int = 30) -> List[Dict]:
    """分批篩選"""
...
<head>
    ...
    <title>台股上市股票篩選報告 - {datetime.now().strftime('%Y-%m-%d %H:%M')}</title>
    <style>
...
<body>
    <div class="container">
        <header>
            <h1>📈 台股上市股票篩選報告</h1>
            <p class="subtitle">篩選時間：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | 使用富邦證券SDK</p>
        </header>
        ...
        <div class="summary">
            ...
            <div class="summary-item">
                <h3 style="color: #667eea">{datetime.now().strftime('%m/%d %H:%M')}</h3>
                <p>報告時間</p>
            </div>
        </div>
...
def save_report(html_content: str) -> str:
    ...
    filename = f"twse_screener_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
...
def main():
    ...
    log(f"分批大小：30 檔/批")
    log(f"預計批次：{(len(TWSE_MAJOR_STOCKS)-1)//30 + 1} 批")
    log("="*60)
    ...
    # 執行篩選
    results = batch_screen(TWSE_MAJOR_STOCKS, batch_size=30)
